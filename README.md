...
Settings → **updates** → `[ check for updates ]`. If a newer release exists it shows the
version, size and release notes, and `[ download & install ]` replaces the running app
and relaunches it. No deleting the old copy, no dragging anything.

It also avoids the Gatekeeper dance. A DMG fetched in a browser arrives with a quarantine
flag, so an ad-hoc signed app out of it needs `xattr -dr com.apple.quarantine` before it
will open. Quarantine is applied by the *downloading* application; the updater fetches
over URLSession and writes the bundle itself, so nothing sets it and the installed copy
just launches.

The swap happens after Deck exits, since the running executable lives inside the bundle
being replaced. The old bundle is moved aside first and put back if the replacement
fails, so an interrupted update leaves a working app rather than none.
...
**On verification:** the updater checks the download against the release's `SHA256SUMS`
and refuses to install on a mismatch. Releases whose checksums file does not list the DMG
— which is every release so far, because the disk image is built and uploaded by hand
while `SHA256SUMS` is generated by the Linux release job — are installed unverified. The
alternative, refusing to update at all, would make the button useless.

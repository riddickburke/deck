...
/// SHA-256 and the base64url encoding that OAuth PKCE requires.
///
/// Written out rather than reached for: CryptoKit is Apple-only and this has to work on
/// the Linux build too, and swift-crypto is a large dependency for one hash. Unlike the
/// cache keys in `StableHash`, this genuinely must be SHA-256 — the value is checked by
/// Spotify's authorization server against the verifier, so any other function fails.
public enum SHA256 {
    ...
    public static func hex(_ message: String) -> String {
        hash(Array(message.utf8)).map { String(format: "%02x", $0) }.joined()
    }
...
public enum PKCE {
    /// base64url of SHA-256(verifier), unpadded, as RFC 7636 specifies.
